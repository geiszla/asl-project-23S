                     ...
                     const int sizer)
{
  ...
  {
    //printf("err_size: %d\n", err_size);
    _mm256_store_pd(&err[i], err_vec);
    _mm256_store_pd(&err[i+4], err_vec);
    _mm256_store_pd(&err[i+8], err_vec);
    _mm256_store_pd(&err[i+12], err_vec);
    _mm256_store_pd(&err[i+16], err_vec);
    _mm256_store_pd(&err[i+20], err_vec);
    _mm256_store_pd(&err[i+24], err_vec);
    _mm256_store_pd(&err[i+28], err_vec);
  }
  for(; i < err_size; i+=1)
  {
    err[i] = 0;
  ...
  {
    _mm256_store_pd(&r_ext[runner], err_vec);
    _mm256_store_pd(&r_ext[runner+4], err_vec);
    _mm256_store_pd(&r_ext[runner+8], err_vec);
    _mm256_store_pd(&r_ext[runner+12], err_vec);
    _mm256_store_pd(&r_ext[runner+16],err_vec);
    _mm256_store_pd(&r_ext[runner+20], err_vec);
    _mm256_store_pd(&r_ext[runner+24], err_vec);
    _mm256_store_pd(&r_ext[runner+28], err_vec);
  }
  ...
  for (int n = 1; n <= (k - 1); n++)
  {
    ...
    {
      _mm256_store_pd(&e_tmp[runner1], err_vec);
      _mm256_store_pd(&p[runner1], err_vec);
      _mm256_store_pd(&e_tmp[runner1+4], err_vec);
      _mm256_store_pd(&p[runner1+4], err_vec);
      _mm256_store_pd(&e_tmp[runner1+8], err_vec);
      _mm256_store_pd(&p[runner1+8], err_vec);
      _mm256_store_pd(&e_tmp[runner1+12], err_vec);
      _mm256_store_pd(&p[runner1+12], err_vec);
      _mm256_store_pd(&e_tmp[runner1+16], err_vec);
      _mm256_store_pd(&p[runner1+16], err_vec);
      _mm256_store_pd(&e_tmp[runner1+20], err_vec);
      _mm256_store_pd(&p[runner1+20], err_vec);
      _mm256_store_pd(&e_tmp[runner1+24], err_vec);
      _mm256_store_pd(&p[runner1+24], err_vec);
      _mm256_store_pd(&e_tmp[runner1+28], err_vec);
      _mm256_store_pd(&p[runner1+28], err_vec);
